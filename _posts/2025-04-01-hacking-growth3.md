...
`그로스 해킹의 핵심`은 `빠른 주기의 실험을 통해 더 많은 통찰을 얻는 것`입니다. 완벽한 실험을 통해 엄청난 결과를 얻을 수 있다고 생각하기 쉽지만, 현실에서는 대부분의 실험이 기대한 결과를 내지 못합니다. 이것이 반복적인 실험과 충분한 실험 횟수가 중요한 이유이며, `그릿(grit)`이 강조되는 이유입니다. 그로스 해킹의 성공은 오랜 시간에 걸쳐 축적된 작은 성공들로부터 비롯됩니다.
...
그로스 해킹은 위 과정이 `반복 순환`되는 과정을 통해 진행됩니다. 우선 각 단계를 살펴본 후, 사례를 바탕으로 그로스 해킹을 진행해도록 하겠습니다.
...
구제척으로 아이디어를 제안하기 위해 다음을 작성하는 것이 좋습니다.
...
- **측정 지표**
    ...
    - 한 지표의 개선이 다른 지표의 하락을 초래할 수 있기 때문에, 실험이 영향을 미치는 하류(downstream) 지표들도 함께 관찰
- **점수 부여**
    - 해당 부분은 아래에서 자세히 다루겠습니다.
...
중요한 점은 `어떤 아이디어가 좋은 아이디어인가에 대한 예측은 언제든 틀릴 수 있다`는 사실입니다. 그래서 낮은 점수를 받은 아이디어라도 섣불리 폐기하지 않는 것이 좋습니다.

### 4단계: 실험
그로스 해킹의 성공을 위해서는 여러 부서 간의 긴밀한 협력이 필수적입니다. 특히 모든 실험이 통계적으로 유의미한 결과를 도출할 수 있도록 설계하는 것이 중요합니다. 이를 위해 다음 두 가지를 고려하여 실험을 진행하는 것이 좋습니다.

* 99%의 통계학적 신뢰 수준 사용
	* 95% 신뢰 수준은 '성공적'이라고 판단된 테스트도 실제로는 잘못된 결과일 가능성이 5퍼센트나 된다는 의미입니다. 따라서 의심이 들 때는 99% 신뢰도를 선택하여 긍정 오류(false positive)를 기반으로 유효하지 않은 실험을 채택할 위험을 줄이는 것이 현명합니다.
* 실험 결과가 모호할 때는 대조군이 항상 이긴다
	* 표본의 크기를 비용효율적인 수준을 넘어서까지 무리하게 확대하지 말고, 실험군과 대조군 사이에 명확한 차이가 없다면 기존 방식을 유지하는 것이 바람직합니다.

실험이 완료되면 실험 결과를 체계적인 `실험 요약서`로 정리해야 합니다. 이 요약서에는 다음과 같은 요소들이 포함되어야 합니다.
...
### 1단계: 분석
초기에 급증했던 사용자 데이터를 자세히 분석하여, 앱을 다운로드한 후 거의 또는 전혀 사용하지 않은 사람들과 정기적으로 쇼핑하는 사용자들의 코호트를 구분했습니다. 이를 통해 특징적인 사용자 집단을 확인하고, 다음과 같은 질문들을 설정합니다.
...
6. **실험 결과**
	- 평균 주문 금액: 실험군이 대조군보다 18% 증가
	- \$50 이상 주문 비율: 실험군이 대조군보다 27% 증가
	- 주문 전환율: 실험군이 대조군보다 5% 증가
...
8. **결론**
	...
	* 특히 장바구니 페이지에서 무료 배송까지 남은 금액을 시각적으로 표시하는 기능이 사용자의 추가 구매 결정에 강한 영향을 미침
		* \$30-49 범위의 장바구니 금액을 가진 사용자들이 무료 배송 기준을 충족하기 위해 추가 구매를 하는 경향이 뚜렷하게 나타남
...
9. **실험을 통해 배운 점**
	...
	* 무료 배송 기준액에 가까운 사용자들이 가장 높은 행동 변화를 보였으므로, 이 세그먼트를 대상으로 한 맞춤형 메시지를 새롭게 제공